...
export default function Home() {
  ...
  const [dashboardTab, setDashboardTab] = useState('donor-avail');
  ...
  const [registerThana, setRegisterThana] = useState('');
  ...
  const [reqThana, setReqThana] = useState('');
    ...
    socket.onmessage = (event) => {
      try {
        ...
        if (data.type === 'CHAT_MSG_INCOMING') {
          ...
          if (chatPartner && (incoming.sender_id === chatPartner.id)) {
            setChatMessages(prev => [...prev, incoming]);
            fetch(`/api/chat/history/${chatPartner.id}`, {
        ...
        } else if (data.type === 'CHAT_MSG_CONFIRM') {
  ...
  const handleMapClick = (title) => {
    setSearchDivision(title);
    setSearchDistrict('');
    setSearchThana('');
    const targetDistrictList = Object.keys(geography[title] || {});
    if (targetDistrictList.length > 0) {
      const firstDist = targetDistrictList[0];
      setSearchDistrict(firstDist);
      const targetThanaList = geography[title][firstDist] || [];
      if (targetThanaList.length > 0) {
        setSearchThana(targetThanaList[0]);
      }
    }
  };
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(data.error || "নিবন্ধন ব্যর্থ হয়েছে");
      }
    } catch (err) {
      alert("নিবন্ধন করতে সমস্যা হচ্ছে");
    }
  };
    ...
    try {
      const res = await fetch('/api/requests/create', {
        ...
        body: JSON.stringify({
          ...
          location_details: reqDetails,
          division: reqDivision,
          district: reqDistrict,
          thana: reqThana,
          urgency_level: reqUrgency
        })
      });
      const data = await res.json();
      if (res.ok) {
        alert("আবেদনটি সফলভাবে পোস্ট করা হয়েছে");
        setPostRequestModal(false);
        loadData();
      } else {
        alert(data.error || "আবেদন পোস্ট করতে ব্যর্থ");
      }
    } catch (err) {
      alert("আবেদন পোস্ট করতে সমস্যা হচ্ছে");
    }
  };
  ...
  const openChatWith = (partner) => {
    ...
    fetch(`/api/chat/history/${partner.id}`, {
      headers: { 'Authorization': `Bearer ${token}` }
    })
      .then(res => res.json())
  ...
  const sendChatMessage = (e) => {
    if (e) e.preventDefault();
    if (!newMessage.trim() || !chatPartner || !wsRef.current) return;
  ...
  return (
    <>
      <Head>
        <title>PulseNetwork - রক্তদাতা ও ব্লাড ব্যাংক নেটওয়ার্ক</title>
        <meta name="description" content="সরাসরি রক্তদাতা ও ব্লাড ব্যাংক ম্যাচিং প্লাটফর্ম" />
      </Head>

      <div className="min-h-screen bg-[#0b0f19] text-[#f3f4f6] pb-10">
        <header className="navbar blood-glass border-b border-red-500/10 px-6 py-4 sticky top-0 z-40">
          <div className="flex-1">
            <a onClick={() => handleNavClick('home')} className="btn btn-ghost text-2xl font-black text-red-500 flex items-center gap-2 cursor-pointer">
              <span className="text-3xl filter drop-shadow-[0_0_8px_rgba(239,68,68,0.6)]">🩸</span> PulseNetwork
            </a>
          </div>
          <div className="flex-none gap-4">
            <ul className="menu menu-horizontal px-1 font-semibold gap-2">
              <li>
                <a className={`hover:text-red-500 transition-colors ${currentView === 'home' ? 'text-red-500 bg-red-500/10' : ''}`} onClick={() => handleNavClick('home')}>
                  হোম
                </a>
              </li>
              <li>
                <a className={`hover:text-red-500 transition-colors ${currentView === 'inventory' ? 'text-red-500 bg-red-500/10' : ''}`} onClick={() => handleNavClick('inventory')}>
                  রক্ত মজুদ
                </a>
              </li>
              <li>
                <a className={`hover:text-red-500 transition-colors ${currentView === 'requests' ? 'text-red-500 bg-red-500/10' : ''}`} onClick={() => handleNavClick('requests')}>
                  জরুরি আবেদন
                </a>
              </li>
              {currentUser && currentUser.role === 'Donor' && (
                <li>
                  <a className={`hover:text-red-500 transition-colors ${currentView === 'dashboard-donor' ? 'text-red-500 bg-red-500/10' : ''}`} onClick={() => handleNavClick('dashboard-donor')}>
                    ড্যাশবোর্ড
                  </a>
                </li>
              )}
              {currentUser && currentUser.role === 'Admin' && (
                <li>
                  <a className={`hover:text-red-500 transition-colors ${currentView === 'dashboard-admin' ? 'text-red-500 bg-red-500/10' : ''}`} onClick={() => handleNavClick('dashboard-admin')}>
                    অ্যাডমিন প্যানেল
                  </a>
                </li>
              )}
            ...
            <div className="flex items-center gap-2">
              {currentUser ? (
                <>
                  <span className="badge badge-error badge-outline p-3 font-bold border-red-500/30 text-red-400 bg-red-500/5">
                    {currentUser.name}
                  </span>
                  <button className="btn btn-outline btn-sm btn-error hover:bg-red-500 hover:text-white" onClick={handleSignOut}>
                    সাইন আউট
                  </button>
                </>
              ) : (
                <>
                  <button className="btn btn-sm btn-ghost hover:bg-slate-800" onClick={() => setAuthModal('login')}>
                    সাইন ইন
                  </button>
                  <button className="btn btn-sm btn-error text-white bg-red-600 hover:bg-red-700 border-none pulse-glow-red" onClick={() => setAuthModal('register')}>
                    নিবন্ধন
                  </button>
                </>
              )}
        ...
        <main className="max-w-7xl mx-auto px-6 mt-8">
          {currentView === 'home' && (
            <div className="space-y-12">
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                <div className="lg:col-span-7 blood-glass rounded-3xl p-6 md:p-8 flex flex-col gap-6 neon-border-red">
                  <div className="flex items-center justify-between">
                    <h2 className="text-xl font-bold text-slate-100 flex items-center gap-2">
                      <span className="text-red-500">●</span> বিভাগীয় মানচিত্র
                    </h2>
                    <span className="text-xs text-slate-400">ফিল্টার করতে বিভাগে ক্লিক করুন</span>
                  </div>
                  <div className="flex justify-center items-center bg-[#070a13] rounded-2xl p-4 border border-slate-800">
                    <svg viewBox="0 0 437.8 601.2" className="w-full max-h-[460px]">
                      {MAP_PATHS.map(path => (
                        <path
                          key={path.id}
                          d={path.d}
                  ...
                  {searchDivision && (
                    <div className="flex items-center justify-between bg-slate-900/50 p-3 rounded-xl border border-red-500/10">
                      <span className="text-sm font-semibold text-slate-300">নির্বাচিত বিভাগ: <span className="text-red-400 font-bold">{searchDivision}</span></span>
                      <button className="btn btn-xs btn-outline btn-error" onClick={() => setSearchDivision('')}>পরিষ্কার করুন</button>
                    </div>
                  )}
                </div>

                <div className="lg:col-span-5 blood-glass border border-slate-800 rounded-3xl p-6 md:p-8 neon-border-red">
                  <h2 className="text-2xl font-black mb-6 text-slate-100 flex items-center gap-2">
                    🔍 dदाता খুঁজুন
                  </h2>
                  <form onSubmit={handleSearch} className="space-y-4">
                    <div className="form-control">
                      <label className="label py-1"><span className="label-text text-slate-300 font-semibold">বিভাগ</span></label>
                      <select value={searchDivision} onChange={(e) => { setSearchDivision(e.target.value); setSearchDistrict(''); setSearchThana(''); }} className="select select-bordered w-full bg-[#121824] border-slate-700 text-slate-100 focus:border-red-500">
                    ...
                    <div className="form-control">
                      <label className="label py-1"><span className="label-text text-slate-300 font-semibold">জেলা</span></label>
                      <select value={searchDistrict} onChange={(e) => { setSearchDistrict(e.target.value); setSearchThana(''); }} className="select select-bordered w-full bg-[#121824] border-slate-700 text-slate-100 focus:border-red-500" disabled={!searchDivision}>
                        <option value="">নির্বাচন করুন</option>
                    ...
                    <div className="form-control">
                      <label className="label py-1"><span className="label-text text-slate-300 font-semibold">থানা</span></label>
                      <select value={searchThana} onChange={(e) => setSearchThana(e.target.value)} className="select select-bordered w-full bg-[#121824] border-slate-700 text-slate-100 focus:border-red-500" disabled={!searchDistrict}>
                        <option value="">নির্বাচন করুন</option>
                    ...
                    <div className="form-control">
                      <label className="label py-1"><span className="label-text text-slate-300 font-semibold">রক্তের গ্রুপ</span></label>
                      <select value={searchBloodGroup} onChange={(e) => setSearchBloodGroup(e.target.value)} required className="select select-bordered w-full bg-[#121824] border-slate-700 text-slate-100 focus:border-red-500">
                        <option value="">নির্বাচন করুন</option>
                        {['A+', 'A-', 'B+', 'B-', 'AB+', 'AB-', 'O+', 'O-'].map(g => <option key={g} value={g}>{g}</option>)}
                      </select>
                    </div>

                    <button type="submit" className="btn btn-error w-full text-white font-bold bg-red-600 hover:bg-red-700 border-none mt-6 pulse-glow-red">
                      অনুসন্ধান করুন
                    </button>
                  </form>
                </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                <div className="blood-glass p-6 rounded-2xl border border-slate-800 text-center hover:border-red-500/20 transition-all">
                  <div className="text-slate-400 text-sm font-semibold">জীবন বেঁচেছে</div>
                  <div className="text-3xl font-black text-red-500 mt-2 filter drop-shadow-[0_0_8px_rgba(239,68,68,0.4)]">{stats.livesSaved}</div>
                </div>
                <div className="blood-glass p-6 rounded-2xl border border-slate-800 text-center hover:border-red-500/20 transition-all">
                  <div className="text-slate-400 text-sm font-semibold">সক্রিয় দাতা</div>
                  <div className="text-3xl font-black text-slate-100 mt-2">{stats.activeDonors}</div>
                </div>
                <div className="blood-glass p-6 rounded-2xl border border-slate-800 text-center hover:border-red-500/20 transition-all">
                  <div className="text-slate-400 text-sm font-semibold">ব্লাড ব্যাংক</div>
                  <div className="text-3xl font-black text-slate-100 mt-2">{stats.bloodBanks}</div>
                </div>
                <div className="blood-glass p-6 rounded-2xl border border-slate-800 text-center hover:border-red-500/20 transition-all">
                  <div className="text-slate-400 text-sm font-semibold">জরুরি রক্ত প্রয়োজন</div>
                  <div className="text-3xl font-black text-red-500 mt-2 filter drop-shadow-[0_0_8px_rgba(239,68,68,0.4)]">{stats.emergencyRequests}</div>
                </div>
              </div>

              {!currentUser && (
                <div className="blood-glass p-6 rounded-2xl border border-slate-800 flex flex-col md:flex-row items-center justify-between gap-4 bg-slate-900/40">
                  <div>
                    <h3 className="font-bold text-slate-200">ডেমো অ্যাকাউন্ট দিয়ে দ্রুত পরীক্ষা করতে চান?</h3>
                    <p className="text-xs text-slate-400 mt-1">নিচের যেকোনো একটি রোলে এক ক্লিকে ডেমো লগইন করুন</p>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    <button onClick={() => handleQuickLogin('donor')} className="btn btn-xs btn-outline btn-error">ডোনার ভিউ</button>
                    <button onClick={() => handleQuickLogin('patient')} className="btn btn-xs btn-outline btn-error">রোগী ভিউ</button>
                    <button onClick={() => handleQuickLogin('hospital')} className="btn btn-xs btn-outline btn-error">ব্লাড ব্যাংক ভিউ</button>
                    <button onClick={() => handleQuickLogin('admin')} className="btn btn-xs btn-outline btn-error">অ্যাডমিন ভিউ</button>
                  </div>
                </div>
              )}

              {searchResults.length > 0 && (
                <div className="blood-glass border border-slate-850 rounded-3xl p-6 md:p-8 neon-border-red">
                  <h3 className="text-2xl font-black text-slate-150 mb-6 flex items-center gap-2">
                    🔍 অনুসন্ধান ফলাফল ({searchResults.length} জন ম্যাচিং দাতা)
                  </h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {searchResults.map(donor => (
                      <div key={donor.user_id} className="blood-glass rounded-2xl border border-slate-800 hover:border-red-500/30 transition-all p-5 flex flex-col justify-between gap-4">
                        <div className="flex justify-between items-start">
                          <div>
                            <h4 className="font-bold text-slate-100 text-lg">{donor.name}</h4>
                            <p className="text-xs text-slate-400 mt-1">{donor.division} → {donor.district} → {donor.thana}</p>
                            <p className="text-xs text-slate-500 mt-1">দূরত্ব: {donor.distance_km} কি.মি.</p>
                          </div>
                          <span className="badge badge-error badge-lg py-4 px-3 font-black text-white text-lg rounded-xl">
                            {donor.blood_group}
                          </span>
                        </div>
                        <div className="flex items-center justify-between border-t border-slate-800 pt-4 mt-2">
                          <span className="text-sm font-semibold text-red-400">📞 {donor.phone}</span>
                          <button onClick={() => openChatWith({ id: donor.user_id, name: donor.name })} className="btn btn-xs btn-error text-white font-bold">
                            মেসেজ করুন
                          </button>
                        </div>
                      </div>
          ...
          {currentView === 'inventory' && (
            <div className="blood-glass border border-slate-800 rounded-3xl p-8 neon-border-red">
              <h2 className="text-3xl font-black text-slate-100">🩸 ব্লাড ব্যাংক রক্ত মজুদ ও ইনভেন্টরি</h2>
              <p className="text-slate-400 text-md mt-1 mb-8">অনুমোদিত ব্লাড ব্যাংকগুলোর লাইভ রক্তের ব্যাগ মজুদ তালিকা</p>

              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                {inventories.map(bank => (
                  <div key={bank.blood_bank_id} className="blood-glass rounded-2xl border border-slate-800 p-6">
                    <h3 className="text-xl font-bold text-slate-200 mb-2">📍 {bank.name}</h3>
                    <p className="text-xs text-slate-400 mb-6">{bank.division} → {bank.district} → {bank.thana} (হটলাইন: {bank.contact_no})</p>
                    <div className="grid grid-cols-4 gap-4">
                      {bank.inventory.map(item => (
                        <div key={item.blood_group} className="bg-slate-900/60 border border-slate-800 p-4 rounded-xl text-center">
                          <div className="text-xs font-black text-slate-400">{item.blood_group}</div>
                          <div className="text-xl font-bold text-red-400 mt-1">{item.quantity} ব্যাগ</div>
                        </div>
                      ))}
                    </div>
                  </div>
          ...
          {currentView === 'requests' && (
            <div className="blood-glass border border-slate-800 rounded-3xl p-8 neon-border-red">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
                <div>
                  <h2 className="text-3xl font-black text-slate-100">🚨 জরুরি রক্তের লাইভ আবেদনসমূহ</h2>
                  <p className="text-slate-400 text-md mt-1">জরুরি রক্তের প্রয়োজনে রোগীদের সাহায্য করতে সরাসরি যোগাযোগ করুন</p>
                </div>
                <button className="btn btn-error text-white font-bold bg-red-600 hover:bg-red-700 border-none pulse-glow-red" onClick={() => setPostRequestModal(true)}>
                  নতুন রক্তের আবেদন
                </button>
              </div>
              ...
              <div className="space-y-4">
                {requests.map(req => (
                  <div key={req.id} className="blood-glass rounded-2xl border border-slate-800 hover:border-red-500/20 transition-all p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                    <div className="space-y-2">
                      <div className="flex items-center gap-3">
                        <span className={`badge ${req.urgency_level === 'High' ? 'badge-error' : 'badge-warning'} text-white font-bold px-3 py-2 text-xs`}>
                          {req.urgency_level === 'High' ? 'অতি জরুরি' : 'জরুরি'}
                        </span>
                        <span className="font-bold text-slate-100 text-lg">{req.hospital_name}</span>
                      </div>
                      <p className="text-sm text-slate-400">লোকেশন: {req.division} → {req.district} → {req.thana} ({req.location_details})</p>
                    </div>
                    <div className="flex items-center gap-8 w-full md:w-auto justify-between md:justify-end border-t md:border-t-0 border-slate-800 pt-4 md:pt-0">
                      <div className="text-right">
                        <div className="text-xs text-slate-500 font-bold">গ্রুপ</div>
                        <div className="text-3xl font-black text-red-500">{req.blood_group}</div>
                      </div>
                      <div className="text-right">
                        <div className="text-xs text-slate-500 font-bold">পরিমাণ</div>
                        <div className="text-xl font-bold text-slate-200">{req.units_needed} ব্যাগ</div>
                      </div>
                      <button onClick={() => openChatWith({ id: req.patient_id, name: "আবেদনকারী রোগী" })} className="btn btn-sm btn-error text-white font-bold">
                        সহায়তা করুন
                      </button>
                    </div>
                  </div>
          ...
          {currentView === 'dashboard-donor' && (
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
              <div className="lg:col-span-1">
                <ul className="menu bg-[#0f172a] border border-slate-800 p-4 rounded-3xl w-full gap-2">
                  <li className="menu-title font-bold text-slate-500 text-xs">দাতা প্যানেল</li>
                  <li>
                    <a className={dashboardTab === 'donor-avail' ? 'active bg-red-600 text-white' : 'text-slate-300'} onClick={() => setDashboardTab('donor-avail')}>
                      স্ট্যাটাস আপডেট
                    </a>
                  </li>
                </ul>
                ...
                <div className="blood-glass border border-slate-800 rounded-3xl p-8 neon-border-red">
                  {dashboardTab === 'donor-avail' && (
                    <div className="space-y-6">
                      <h3 className="text-2xl font-black text-slate-100">রক্তদান স্ট্যাটাস ও প্রাপ্যতা</h3>
                      <p className="text-slate-400 text-md">আপনার সার্চ ভিজিবিলিটি আপডেট করুন। এটি বন্ধ থাকলে রোগীরা আপনাকে তালিকায় দেখতে পাবেন না।</p>
                      <div className="bg-[#121824] border border-slate-800 p-6 rounded-2xl flex flex-row justify-between items-center">
                        <div>
                          <h4 className="font-bold text-slate-200 text-lg">অনুসন্ধানে প্রাপ্যতা</h4>
                          <p className="text-xs text-slate-400 mt-1">প্রাপ্যতা অন থাকলে রোগীরা জরুরি মুহূর্তে আপনাকে খুঁজে পাবে।</p>
                        </div>
                        <span className="badge badge-success text-white font-bold p-4">সক্রিয় (Active)</span>
                      </div>
                    </div>
                  )}
                </div>
          ...
          {currentView === 'dashboard-admin' && (
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
              <div className="lg:col-span-1">
                <ul className="menu bg-[#0f172a] border border-slate-800 p-4 rounded-3xl w-full gap-2">
                  <li className="menu-title font-bold text-slate-500 text-xs">কন্ট্রোল প্যানেল</li>
                  <li>
                    <a className={dashboardTab === 'admin-users' ? 'active bg-red-600 text-white' : 'text-slate-300'} onClick={() => setDashboardTab('admin-users')}>
                      ইউজার ম্যানেজমেন্ট
                    </a>
                  </li>
                </ul>
              ...
              <div className="lg:col-span-3">
                <div className="blood-glass border border-slate-800 rounded-3xl p-8 neon-border-red">
                  {dashboardTab === 'admin-users' && (
                    <div className="space-y-6">
                      <h3 className="text-2xl font-black text-slate-100">নিবন্ধিত ব্যবহারকারী তালিকা</h3>
                      <div className="overflow-x-auto border border-slate-800 rounded-2xl bg-[#0f172a]/55">
                        <table className="table table-zebra w-full text-slate-300">
                          ...
                          <tbody>
                            {users.map(u => (
                              <tr key={u.id} className="hover:bg-slate-900/60 border-b border-slate-800/50">
                                <td className="font-semibold text-slate-100">{u.name}</td>
                                <td>{u.email}</td>
                                <td><span className="badge badge-ghost font-bold text-xs">{u.role}</span></td>
                                <td>{u.division}</td>
                              </tr>
                            ))}
                            {users.length === 0 && (
                              <tr>
                                <td colSpan="4" className="text-center py-6 text-slate-500">কোন ব্যবহারকারী পাওয়া যায়নি</td>
                              </tr>
                            )}
                          </tbody>
                        </table>
        ...
        {authModal && (
          <div className="modal modal-open">
            <div className="modal-box bg-[#0f172a] max-w-[460px] relative border border-slate-800 shadow-2xl rounded-3xl">
              <button className="btn btn-sm btn-circle btn-ghost absolute right-4 top-4 text-slate-400" onClick={() => setAuthModal(null)}>✕</button>

              <div className="tabs tabs-boxed mb-6 justify-center bg-slate-900/60 p-1 border border-slate-800">
                <a className={`tab font-bold text-sm rounded-xl px-6 py-2 transition-all ${authModal === 'login' ? 'bg-red-600 text-white' : 'text-slate-400'}`} onClick={() => setAuthModal('login')}>সাইন ইন</a>
                <a className={`tab font-bold text-sm rounded-xl px-6 py-2 transition-all ${authModal === 'register' ? 'bg-red-600 text-white' : 'text-slate-400'}`} onClick={() => setAuthModal('register')}>নিবন্ধন</a>
              </div>
              ...
              {authModal === 'login' ? (
                <form onSubmit={handleLogin} className="space-y-4">
                  <div className="form-control">
                    ...
                    <input type="email" value={loginEmail} onChange={(e) => setLoginEmail(e.target.value)} required placeholder="email@example.com" className="input input-bordered bg-[#121824] border-slate-700 w-full text-slate-100" />
                  </div>
                  <div className="form-control">
                    <label className="label"><span className="label-text font-bold text-slate-300">পাসওয়ার্ড</span></label>
                    <input type="password" value={loginPassword} onChange={(e) => setLoginPassword(e.target.value)} required placeholder="••••••••" className="input input-bordered bg-[#121824] border-slate-700 w-full text-slate-100" />
                  </div>
                  <button type="submit" className="btn btn-error w-full text-white font-bold mt-6 bg-red-600 hover:bg-red-700 border-none pulse-glow-red">
                    লগইন করুন
                  </button>
                </form>
              ) : (
                <form onSubmit={handleRegister} className="space-y-4 max-h-[500px] overflow-y-auto pr-2">
                  <div className="form-control">
                    ...
                    <input type="text" value={registerName} onChange={(e) => setRegisterName(e.target.value)} required placeholder="যেমন: তানভীর রহমান" className="input input-bordered bg-[#121824] border-slate-700 w-full text-slate-100" />
                  </div>
                  <div className="form-control">
                    <label className="label"><span className="label-text font-bold text-slate-300">ইমেইল</span></label>
                    <input type="email" value={registerEmail} onChange={(e) => setRegisterEmail(e.target.value)} required placeholder="email@example.com" className="input input-bordered bg-[#121824] border-slate-700 w-full text-slate-100" />
                  </div>
                  <div className="form-control">
                    <label className="label"><span className="label-text font-bold text-slate-300">পাসওয়ার্ড</span></label>
                    <input type="password" value={registerPassword} onChange={(e) => setRegisterPassword(e.target.value)} required placeholder="••••••••" className="input input-bordered bg-[#121824] border-slate-700 w-full text-slate-100" />
                  </div>
                  <div className="form-control">
                    <label className="label"><span className="label-text font-bold text-slate-300">ফোন নম্বর</span></label>
                    <input type="text" value={registerPhone} onChange={(e) => setRegisterPhone(e.target.value)} required placeholder="+88017XXXXXXXX" className="input input-bordered bg-[#121824] border-slate-700 w-full text-slate-100" />
                  </div>
                  <div className="form-control">
                    <label className="label"><span className="label-text font-bold text-slate-300">জাতীয় পরিচয়পত্র / জন্ম সনদ</span></label>
                    <input type="text" value={registerNid} onChange={(e) => setRegisterNid(e.target.value)} required placeholder="NID বা জন্ম সনদ নম্বর" className="input input-bordered bg-[#121824] border-slate-700 w-full text-slate-100" />
                  </div>
                  <div className="form-control">
                    <label className="label"><span className="label-text font-bold text-slate-300">বিভাগ</span></label>
                    <select value={registerDivision} onChange={(e) => { setRegisterDivision(e.target.value); setRegisterDistrict(''); setRegisterThana(''); }} required className="select select-bordered bg-[#121824] border-slate-700 w-full text-slate-100">
                      <option value="">নির্বাচন করুন</option>
                      {divisionList.map(div => <option key={div} value={div}>{div}</option>)}
                    </select>
                  </div>
                  <div className="form-control">
                    ...
                    <select value={registerDistrict} onChange={(e) => { setRegisterDistrict(e.target.value); setRegisterThana(''); }} required className="select select-bordered bg-[#121824] border-slate-700 w-full text-slate-100" disabled={!registerDivision}>
                      <option value="">নির্বাচন করুন</option>
                  ...
                  <div className="form-control">
                    ...
                    <select value={registerThana} onChange={(e) => setRegisterThana(e.target.value)} required className="select select-bordered bg-[#121824] border-slate-700 w-full text-slate-100" disabled={!registerDistrict}>
                      <option value="">নির্বাচন করুন</option>
                  ...
                  <div className="form-control">
                    ...
                    <select value={registerRole} onChange={(e) => setRegisterRole(e.target.value)} className="select select-bordered bg-[#121824] border-slate-700 w-full text-slate-100">
        ...
        {postRequestModal && (
          <div className="modal modal-open">
            <div className="modal-box bg-[#0f172a] max-w-[500px] relative border border-slate-800 shadow-2xl rounded-3xl">
              ...
              <form onSubmit={handleCreateRequest} className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="form-control">
                    <label className="label"><span className="label-text text-slate-300 font-semibold">রক্তের গ্রুপ</span></label>
                    <select value={reqBloodGroup} onChange={(e) => setReqBloodGroup(e.target.value)} className="select select-bordered bg-[#121824] border-slate-700 text-slate-100">
                      {['A+', 'A-', 'B+', 'B-', 'AB+', 'AB-', 'O+', 'O-'].map(g => <option key={g} value={g}>{g}</option>)}
                    </select>
                  </div>
                  <div className="form-control">
                    <label className="label"><span className="label-text text-slate-300 font-semibold">পরিমাণ (ব্যাগ)</span></label>
                    <input type="number" min="1" max="10" value={reqUnits} onChange={(e) => setReqUnits(e.target.value)} className="input input-bordered bg-[#121824] border-slate-700 text-slate-100" />
                  </div>
                </div>

                <div className="form-control">
                  <label className="label"><span className="label-text text-slate-300 font-semibold">হাসপাতাল / ব্লাড ব্যাংকের নাম</span></label>
                  <input type="text" value={reqHospital} onChange={(e) => setReqHospital(e.target.value)} required placeholder="যেমন: ঢাকা মেডিকেল কলেজ হাসপাতাল" className="input input-bordered bg-[#121824] border-slate-700 text-slate-100" />
                </div>

                <div className="form-control">
                  <label className="label"><span className="label-text text-slate-300 font-semibold">নির্দিষ্ট ওয়ার্ড / কেবিন নম্বর</span></label>
                  <input type="text" value={reqDetails} onChange={(e) => setReqDetails(e.target.value)} placeholder="যেমন: ওয়ার্ড ৫, বেড ১০" className="input input-bordered bg-[#121824] border-slate-700 text-slate-100" />
                </div>

                <div className="form-control">
                  <label className="label"><span className="label-text text-slate-300 font-semibold">বিভাগ</span></label>
                  <select value={reqDivision} onChange={(e) => { setReqDivision(e.target.value); setReqDistrict(''); setReqThana(''); }} required className="select select-bordered bg-[#121824] border-slate-700 text-slate-100">
                    <option value="">নির্বাচন করুন</option>
                    {divisionList.map(div => <option key={div} value={div}>{div}</option>)}
                  </select>
                </div>
                ...
                <div className="form-control">
                  <label className="label"><span className="label-text text-slate-300 font-semibold">জেলা</span></label>
                  <select value={reqDistrict} onChange={(e) => { setReqDistrict(e.target.value); setReqThana(''); }} required className="select select-bordered bg-[#121824] border-slate-700 text-slate-100" disabled={!reqDivision}>
                    <option value="">নির্বাচন করুন</option>
                ...
                <div className="form-control">
                  <label className="label"><span className="label-text text-slate-300 font-semibold">থানা</span></label>
                  <select value={reqThana} onChange={(e) => setReqThana(e.target.value)} required className="select select-bordered bg-[#121824] border-slate-700 text-slate-100" disabled={!reqDistrict}>
                    <option value="">নির্বাচন করুন</option>
                ...
                <div className="form-control">
                  <label className="label"><span className="label-text text-slate-300 font-semibold">জরুরি স্তর</span></label>
                  <select value={reqUrgency} onChange={(e) => setReqUrgency(e.target.value)} className="select select-bordered bg-[#121824] border-slate-700 text-slate-100">
        ...
        {chatPartner && (
          <div className="fixed bottom-0 right-6 w-80 h-96 bg-[#0f172a] border border-red-500/20 rounded-t-3xl shadow-2xl flex flex-col z-50 overflow-hidden">
            <div className="bg-[#121824] border-b border-slate-800 p-4 flex justify-between items-center">
              <div className="flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-green-500"></span>
                <span className="font-bold text-sm text-slate-200">{chatPartner.name}</span>
              </div>
              <button className="btn btn-xs btn-circle btn-ghost text-slate-400" onClick={() => setChatPartner(null)}>✕</button>
            </div>

            <div className="flex-1 p-4 overflow-y-auto space-y-3 bg-[#070a13]/65">
              {chatMessages.map(msg => (
                <div key={msg.id} className={`chat ${msg.sender_id === currentUser.id ? 'chat-end' : 'chat-start'}`}>
                  <div className={`chat-bubble text-xs ${msg.sender_id === currentUser.id ? 'bg-red-600 text-white' : 'bg-slate-800 text-slate-200'}`}>
                    {msg.message}
                  </div>
                </div>
              ))}
            ...
            <form onSubmit={sendChatMessage} className="p-3 bg-[#121824] border-t border-slate-800 flex gap-2">
              <input
                type="text"
                value={newMessage}
                onChange={(e) => setNewMessage(e.target.value)}
                placeholder="মেসেজ লিখুন..."
                className="input input-sm input-bordered bg-[#070a13] border-slate-700 flex-1 text-slate-200 text-xs focus:border-red-500"
              />
              <button type="submit" className="btn btn-sm btn-error text-white font-bold bg-red-600 hover:bg-red-700 border-none px-4">
                পাঠান
              </button>
            </form>
          </div>
